      ...
      <div id="body">
        ...
        <form id="contribution">
          ...
          ${e.length?`<fieldset id="options">
              ...
              ${e.reduce((s,y)=>{const E=`<label class="button">${I.contributionOption(S(y))}<input type="radio" name="options" value="${y}" ${parseInt(y,10)===x?'checked="true"':""} ${u?"":"required"} /></label>`;return s+E},"")}
            </fieldset>`:""}
        <fieldset id="toggle">
            ${u?`<label class="button">${I.cunstomAmountLabel}
                <input id="amount" name="amount" type="number" min="1" placeholder="${I.customAmountPlaceholder}" ${u?"required":""} />
