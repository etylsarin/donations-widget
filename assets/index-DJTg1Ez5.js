(function(){const L=document.createElement("link").relList;if(L&&L.supports&&L.supports("modulepreload"))return;for(const u of document.querySelectorAll('link[rel="modulepreload"]'))M(u);new MutationObserver(u=>{for(const j of u)if(j.type==="childList")for(const A of j.addedNodes)A.tagName==="LINK"&&A.rel==="modulepreload"&&M(A)}).observe(document,{childList:!0,subtree:!0});function t(u){const j={};return u.integrity&&(j.integrity=u.integrity),u.referrerPolicy&&(j.referrerPolicy=u.referrerPolicy),u.crossOrigin==="use-credentials"?j.credentials="include":u.crossOrigin==="anonymous"?j.credentials="omit":j.credentials="same-origin",j}function M(u){if(u.ep)return;u.ep=!0;const j=t(u);fetch(u.href,j)}})();const E=i=>i.replace(/-./g,L=>L[1].toUpperCase()),S=i=>i.replace(new RegExp("(?<!\\.\\d+)\\B(?=(\\d{3})+\\b)","g")," ").replace(new RegExp("(?<=\\.(\\d{3})+)\\B","g")," "),c=()=>{var i;return(i=(+performance.now()*1e9).toFixed().match(/.{1,15}/g))==null?void 0:i[0]},a=["start-date","total-contribution","total-contributors","currency","contribution-options","lang","recurrent"],g=class g extends HTMLElement{constructor(){super(),this.state={amount:0,contributionOptions:[],showRecurrent:!1,showCustomAmount:!0,isDone:!1,isError:!1},this.shadow=null}static get observedAttributes(){return a}attributeChangedCallback(L,t,M){if(t===M)return;const u={};switch(L){case"contribution-options":u.contributionOptions=M.split(","),u.showCustomAmount=!u.contributionOptions.length;break;case"start-date":u.startDate=new Date(M);break;case"total-contribution":u.totalContribution=S(M);break;case"lang":u.lang=M.toLocaleLowerCase();break;case"recurrent":u.showRecurrent=M==="true";break;default:u[E(L)]=M}this.state={...this.state,...u}}setisRecurrent(L){this.state.isRecurrent=L==="true"}setCustomAmount(L){this.state.amount=parseInt(L,10)||0}setCustomAmountFocus(){var L,t;(t=(L=this.shadow)==null?void 0:L.querySelector("#amount"))==null||t.focus()}setCustomAmountToggle(L){this.state.showCustomAmount=L}resetRadios(){var t;const L=(t=this.shadow)==null?void 0:t.querySelector('input[name="options"]:checked');L&&(L.checked=!1)}updateSubmitButtonText(){var M;const L=g.getTranslations(this.state.lang),t=(M=this.shadow)==null?void 0:M.getElementById("submit");t&&(t.textContent=`${L.donateButton} ${this.state.amount?L.contributionOption(S(`${this.state.amount}`)):""}`)}async sendDonation(){const L=g.getTranslations(this.state.lang),M=await(await fetch("https://wazxcc9io0.execute-api.eu-central-1.amazonaws.com/integration/hh_set_config",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({parameters:{amount:this.state.amount*100,currency:L.currencyCode,orderNumber:c(),redirect_url:window.location.href.split("?")[0]}})})).text();window.location.replace(M)}render(){const t=new URLSearchParams(window.location.search).get("RESULTTEXT");t&&(t==="OK"?this.state.isDone=!0:this.state.isError=!0),this.shadow&&(this.shadow.innerHTML=g.getHTML(this.state))}connectedCallback(){this.shadow=this.attachShadow({mode:"closed"}),this.render(),this.shadow.addEventListener("click",L=>{switch(L.target.id){case"customFieldToggle":this.setCustomAmountToggle(!0),this.render(),this.setCustomAmountFocus(),this.updateSubmitButtonText();break}}),this.shadow.addEventListener("submit",L=>{switch(L.preventDefault(),L.target.id){case"contribution":this.sendDonation();break}}),this.shadow.addEventListener("keydown",L=>{switch(L.target.id){case"amount":["e","E","+","-"].includes(L.key)&&L.preventDefault();break}}),this.shadow.addEventListener("input",L=>{switch(L.target.id){case"amount":this.setCustomAmount(L.target.value),this.updateSubmitButtonText(),this.resetRadios();break}}),this.shadow.addEventListener("change",L=>{switch(L.target.name){case"repetition":this.setisRecurrent(L.target.value);break;case"options":this.setCustomAmount(L.target.value),this.setCustomAmountToggle(!1),this.render(),this.updateSubmitButtonText();break}})}};g.getTranslations=(L="en-us")=>{const t={"cs-cz":{infoStartDate:M=>`vybíráme od ${M.toLocaleDateString(L)}`,infoContributions:M=>`${M} ${t[L].currencySymbol}`,infoContributors:M=>`${M} lidí`,labelContributors:"přispělo",once:"jednorázově",recurrent:"měsíčně",presetOptionsLegend:"Přispět",cunstomAmountLabel:"Kolik chcete přispět?",contributionOption:M=>`${M} ${t[L].currencySymbol}`,customAmountPlaceholder:"Kč",customAmountButton:"Jiná částka",donateButton:"Darovat",currencySymbol:"Kč",currencyCode:"203",successMessage:"Děkujeme za váš příspěvek pro HappyHearts Czech Republic! Vaši platbu jsme přijali v pořádku.",errorMessage:"Nepodařilo se dokončit platbu. Zkuste to znovu."},"en-us":{infoStartDate:M=>`campaign started on ${M.toLocaleDateString(L)}`,infoContributions:M=>`${t[L].currencySymbol} ${M}`,infoContributors:M=>`${M} people`,labelContributors:"donated",once:"once",recurrent:"monthly",presetOptionsLegend:"Donate",cunstomAmountLabel:"How much to donate?",contributionOption:M=>`${t[L].currencySymbol}${M}`,customAmountPlaceholder:"$",customAmountButton:"Other Amount",donateButton:"Donate",currencySymbol:"$",currencyCode:"200",successMessage:"Thank you for supporting HappyHearts Czech Republic! Your donation was received.",errorMessage:"We could not process your donation. Try again."}};return t[L]},g.getHTML=({isDone:L,isError:t,showRecurrent:M,showCustomAmount:u,isRecurrent:j,startDate:A,totalContribution:D,totalContributors:C,contributionOptions:e,amount:x,lang:o})=>{const T="data:image/svg+xml;base64,",I=g.getTranslations(o);return`
    <style>
    #widget-wrapper {
      font-family: "Open Sans", "Segoe UI", Tahoma, sans-serif;
      background: #fcfcfc;
      border-radius: 24px;
      min-width: 540px;
    }
    #body {
      border-radius: 24px;
      padding: 32px;
      box-shadow: 0px 6px 14px 0px rgba(0,0,0,0.2);
    }
    #footer {
      padding: 24px 32px;
      display: flex;
      align-items: center;
      justify-content: space-between;
    }
    #footer img {
      width: 184px;
      height: 61px;
    }
    #footer p {
      color: #F26538;
      font-weight: 500;
      font-size: 14px;
    }
    #info {
      margin-bottom: 50px;
      display: flex;
      justify-content: space-between;
      align-items: flex-end;
      gap: 20px;
    }
    p {
      margin: 0;
      color: #555;
      font-size: 16px;
      line-height: 20px;
    }
    #contributions {
      font-size: 48px;
      font-weight: 700;
      line-height: 60px;
      color: #222;
    }
    #contributors {
      color: #222;
      font-size: 32px;
      line-height: 44px;
      margin-bottom: 5px;
    }
    fieldset {
      border: 0;
      margin: 0;
      padding: 0;
      display: flex;
      gap: 12px;
      margin-bottom: 36px;
    }
    legend {
      font-size: 20px;
      line-height: 28px;
      color: #555;
      margin-bottom: 16px;
    }
    .button {
      background: linear-gradient(#fcfcfc, #fcfcfc) padding-box,
                  linear-gradient(#ddd, #ddd) border-box;
      border-radius: 32em;
      border: 2px solid transparent;
      padding: 12px 24px;
      cursor: pointer;
      font-size: 20px;
      line-height: 28px;
      display: flex;
      align-items: center;
      justify-content: center;
      gap: 12px;
      flex-grow: 1;
      text-align: center;
      transition: flex-basis 100ms ease-out;
      position: relative;
    }
    .button:has(input:checked) {
      color: #F26538;
      background: linear-gradient(white, white) padding-box,
                  linear-gradient(90deg, #F26538 0%, #FBBE5E 100%) border-box;

  }
  .button:has(input:checked)::before {
    content: '';
    display: inline-block;
    width: 20px;
    height: 20px;
    background-image: url(data:image/svg+xml;base64,);
  }
  .button input[type="radio"] {
      opacity: 0;
      position: absolute;
      left: 50%;
      transform: translateX(-100%);
  }
  #customFieldToggle {
    width: 100%;
  }
    button {
        cursor: pointer;
    }
    button[type="submit"] {
      background: linear-gradient(90deg, #F26538 0%, #FBBE5E 100%);
      border-radius: 12px;
      padding: 24px;
      color: #1A1A1A;
      font-size: 32px;
      line-height: 44px;
      font-weight: 700;
      border: 0;
      width: 100%;
    }
    input[type=number]::-webkit-outer-spin-button,
    input[type=number]::-webkit-inner-spin-button {
        -webkit-appearance: none;
        margin: 0;
    }
    input[type=number] {
        -moz-appearance: textfield;
        border: 0;
        background: transparent;
        font-size: 20px;
        line-height: 28px;
        flex-grow: 1;
        flex-shrink: 1;
    }
    input[type=number]:focus {
      outline: none;
    }
    input[type=number]::placeholder {
      text-align: right; 
    }
    #repetition .button {
      flex-basis: 50%;
    }
    #repetition .button:has(input:checked) {
      flex-basis: 60%;
    }
    #options {
      margin-bottom: 24px;
    }
    #options .button {
      flex-basis: 33%;
    }
    #options .button:has(input:checked) {
      flex-basis: 45%;
    }
    #response {
      margin-top: 32px;
      border: 2px solid transparent;
      border-radius: 12px;
      padding: 16px;
    }
    #response.success {
      border-color: #30D053;
    }
    #response.error {
      border-color: #F23838;
      color: #F23838;
    }
    </style>
    <div id="widget-wrapper">
      <div id="body">
        <div id="info">
          <div>  
            ${D?`<p id="contributions">${I.infoContributions(D)}</p>`:""}
            ${A?`<p id="date">${I.infoStartDate(A)}</p>`:""}
          </div>
          <div>
            ${C?`<p id="contributors">${I.infoContributors(C)}</p><p>${I.labelContributors}</p>`:""}
          </div>
        </div>
        <form id="contribution">
          ${M?`<fieldset id="repetition">
            <label class="button">${I.once}<input type="radio" name="repetition" value="false" ${j===!1?'checked="true"':""} /></label>
            <label class="button">${I.recurrent}<input type="radio" name="repetition" value="true" ${j===!0?'checked="true"':""} /></label>
          </fieldset>`:""}
          ${e.length?`<fieldset id="options">
              <legend>${I.presetOptionsLegend}</legend>
              ${e.reduce((s,y)=>{const n=`<label class="button">${I.contributionOption(S(y))}<input type="radio" name="options" value="${y}" ${parseInt(y,10)===x?'checked="true"':""} ${u?"":"required"} /></label>`;return s+n},"")}
            </fieldset>`:""}
        <fieldset id="toggle">
            ${u?`<label class="button">${I.cunstomAmountLabel}
                <input id="amount" name="amount" type="number" min="1" max="1000000" step="1" placeholder="${I.customAmountPlaceholder}" ${u?"required":""} />
              </label>`:`<button class="button" type="button" id="customFieldToggle">${I.customAmountButton}</button>`}
          </fieldset>
          <button type="submit" id="submit">${I.donateButton}</button>
        </form>
        ${L?`<div id="response" class="success">${I.successMessage}</div>`:""}
        ${t?`<div id="response" class="error">${I.errorMessage}</div>`:""}
      </div>
      <div id="footer">
        <img src="${T}" />
        <p>happyheartsczech.org</p>
      </div>
    </div>`};let N=g;customElements.define("donations-widget",N);
