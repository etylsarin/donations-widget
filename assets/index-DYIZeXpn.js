    ...
    <div id="widget-wrapper">
      <div id="body">
        <div id="info">
          ${A?`
          <div>
            ${A?`<p id="contributions">${I.infoContributions(A)}</p>`:""}
            ${C?`<p id="date">${I.infoStartDate(C)}</p>`:""}
          </div>
          <div>
            ${T?`<p id="contributors">${I.infoContributors(T)}</p><p>${I.labelContributors}</p>`:""}
          </div>
          `:`<img class="logo" src="${E}" alt="Happy Hearts" />`}
        </div>
        <form id="contribution">
          ...
          ${e.length?`<fieldset id="options">
              ...
              ${e.reduce((s,D)=>{const n=`<label class="button">${I.contributionOption(i(D))}<input type="radio" name="options" value="${D}" ${parseInt(D,10)===S?"checked":""} ${t?"":"required"} /></label>`;return s+n},"")}
            </fieldset>`:""}
        <fieldset id="toggle">
            ${t?`<label class="button">${I.cunstomAmountLabel}
                <input id="amount" name="amount" type="number" min="1" max="1000000" step="1" placeholder="${I.customAmountPlaceholder}" ${t?"required":""} />
              </label>`:`<button class="button" type="button" id="customFieldToggle">${I.customAmountButton}</button>`}
          </fieldset>
          <button type="submit" id="submit" ${L?"disabled":""}>${L?'<span class="loader"></span>':`${I.donateButton}  ${S?I.contributionOption(i(`${S}`)):""}`}</button>
        </form>
        ${M?`<div id="response" class="success">${I.successMessage}</div>`:""}
        ${u?`<div id="response" class="error">${I.errorMessage}</div>`:""}
        <div class="logos">
          <img src="${z}" alt="Mastercard" />
          <img src="${r}" alt="Visa" />
          <img src="${Y}" alt="American Express" />
          <img src="${d}" alt="Apple Pay" />
          <img src="${l}" alt="Google Pay" />
        </div>
        <label class="opt-in"><input type="checkbox" name="marketing" value="opt-in" /> ${I.newsletterOptIn}</label>
      </div>
      <div id="footer">
        ${A?`<img class="logo" src="${E}" alt="Happy Hearts" />`:""}
        <p class="url">happyheartsczech.org</p>
        <p class="author">Brought to you by <img src="${z}" alt="Mastercard" /></p>
      </div>
    </div>`};let x=N;customElements.define("donations-widget",x);
