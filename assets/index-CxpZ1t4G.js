      ...
      <div id="body">
        ...
        <form id="contribution">
          ...
          ${e.length?`<fieldset id="options">
              ...
              ${e.reduce((s,D)=>{const w=`<label class="button">${I.contributionOption(S(D))}<input type="radio" name="options" value="${D}" ${parseInt(D,10)===i?"checked":""} ${t?"":"required"} /></label>`;return s+w},"")}
            </fieldset>`:""}
        <fieldset id="toggle">
          ...
          <label class="opt-in"><input type="checkbox" name="marketing" value="opt-in" /> I want to recieve news</label>
          <button type="submit" id="submit" ${L?"disabled":""}>${L?'<span class="loader"></span>':`${I.donateButton}  ${i?I.contributionOption(S(`${i}`)):""}`}</button>
        </form>
        ${M?`<div id="response" class="success">${I.successMessage}</div>`:""}
        ${u?`<div id="response" class="error">${I.errorMessage}</div>`:""}
        <div class="logos">
          <img src="${o}" alt="Mastercard" />
          <img src="${Y}" alt="Visa" />
          <img src="${r}" alt="American Express" />
          <img src="${d}" alt="Apple Pay" />
          <img src="${m}" alt="Google Pay" />
        </div>
      </div>
      <div id="footer">
        ${A?`<img class="logo" src="${E}" alt="Happy Hearts" />`:""}
        <p class="url">happyheartsczech.org</p>
        <p class="author">Brought to you by <img src="${o}" alt="Mastercard" /></p>
      </div>
